...
export interface DecisionWorkspacePanelProps {
  ...
  cooperationData?: import('@/services/api/types').CooperationResponse;
...
export function DecisionWorkspacePanel({
  ...
  return (
    ...
    >
      ...
      {/* Mobility Cooperation -- Real-Time Negotiation Pivot */}
              ...
              {cooperationData.candidates.length > 0 ? (
                cooperationData.candidates.map((cand, idx) => (
                  <div key={cand.commuter_id} className={cn("rounded-xl border border-white/20 p-3 flex flex-col gap-2", idx === 0 ? "bg-[#4D7C3E]/20" : "bg-white/5")}>
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-white">{cand.cooperation_type === 'shared_ride' ? 'Shared Ride' : cand.cooperation_type === 'relay' ? 'Relay' : 'Shared First Leg'} with {cand.commuter_name}</span>
