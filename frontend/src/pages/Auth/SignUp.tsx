  ...
  return (
    <div className="flex-1 w-full max-w-md mx-auto px-4 py-12 flex flex-col justify-center">
      <div className="bg-black/40 backdrop-blur-xl border border-white/20 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
        {/* Decorative elements */}
        <div className="absolute top-0 left-0 -ml-16 -mt-16 w-32 h-32 bg-[#8EE074]/30 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 right-0 -mr-16 -mb-16 w-32 h-32 bg-blue-500/20 rounded-full blur-3xl pointer-events-none" />
        
        <div className="relative z-10 text-center mb-8">
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Create Account</h1>
          <p className="text-white/70 mt-2 text-sm">Join the movement to a greener city.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="relative z-10 flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-white/80 uppercase tracking-wider ml-1">Full Name</label>
            <input 
              type="text" 
              required 
              value={name}
        ...
        <p className="mt-8 text-center text-sm text-white/60 relative z-10">
          ...
          <Link to="/login" className="text-[#8EE074] font-semibold hover:underline">
            Sign In
          </Link>
        </p>
      </div>
