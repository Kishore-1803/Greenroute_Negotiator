...
_AGENT_VOICE = {
    ...
    "weather": "Weather Agent, who argues for enclosed modes (like car) during rain or bad weather, and open/active modes during pleasant weather.",
}
...
def _facts_block(context: NegotiationContext) -> dict:
    ...
    if context.weather:
        result["weather"] = context.weather
    return result
...
def round_1_system_prompt(agent: str) -> str:
    n_others = len(_AGENT_VOICE) - 1
    return (
        f"You are the {_AGENT_VOICE[agent]} You are in Round 1 of a 2-round negotiation with {n_others} other "
        f"specialist agents (arguing for the other metrics) over which transport mode a traveller should "
...
def round_2_system_prompt(agent: str) -> str:
    return (
        f"You are the {_AGENT_VOICE[agent]} This is Round 2. You have now seen the other agents' Round 1 "
